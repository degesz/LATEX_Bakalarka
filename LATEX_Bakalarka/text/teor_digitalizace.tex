...
Výstupem samovyvažovacího můstku jsou dva harmonické signály 
...
\end{equation}
\begin{equation}
    \varphi = \operatorname{atan2}(-U_s, U_c)
    \label{eq:phase_dft}
\end{equation}
...
\begin{equation}
    |Z| = \frac{A_u}{A_i}
    \label{eq:impedance_modulus}
\end{equation}

Komplexní impedance je následně získána spojením modulu a fázového posunu:
\begin{equation}
    Z = |Z| \, e^{j\Delta\varphi} = |Z| \bigl(\cos\Delta\varphi + j\sin\Delta\varphi\bigr)
