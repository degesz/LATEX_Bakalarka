...
Existuje mnoho metod pro měření impedance, z nichž každá má své výhody a nevýhody. \cite{Steinbauer_Mereni2010} 
Neexistuje jediná nejlepší metoda -- volba závisí na požadované přesnosti, 
měřeném frekvenčním rozsahu, typu měřené impedance a dalších faktorech.

Níže je popsáno několik metod, které se v běžné laboratorní praxi používají. 
Dále existuje několik specializovaných metod pro měření impedance při rádiových 
frekvencích, které zde nejsou zmíněny.
...
\begin{figure}[h!]
    ...
    \caption{Schéma měřicího můstku}
...
\subsection{Samovyvažovací můstek}
V praxi se metoda měření proudu a napětí často implementuje pomocí tzv. samovyvažovacího můstku,
...
\begin{figure}[h!]
    ...
    \caption{Schéma samovyvažovacího můstku}
...
a možnost nastavit parametry měření jako frekvenci měřicího signálu. \cite{Keysight_U1732C_Datasheet}
...
měřicích sond. Na rozdíl od ručních měřičů se zde používají stíněné koaxiální sondy, které jsou méně náchylné k rušení.
...
\section{Závěr z teoretické části}
Na základě rešerše komerčně dostupných přístrojů \cite{Keysight_U1732C_Datasheet} \cite{RS_LCX_Datasheet} byly stanoveny 
klíčové parametry navrhovaného zařízení. Cílem je vytvořit přístroj, který se svými vlastnostmi přiblíží 
profesionálním řešením, avšak s výrazně nižšími pořizovacími náklady. 

Specifikace navrhovaného zařízení:
\begin{itemize}
    \item \textbf{Frekvenční rozsah:} $100~\mathrm{Hz}$ -- $100~\mathrm{kHz}$
    \item \textbf{Rozsah měření indukčnosti:} $100~\mathrm{nH}$ -- $10~\mathrm{H}$
    \item \textbf{Rozsah měření kapacity:} $100~\mathrm{pF}$ -- $10~\mathrm{mF}$
    \item \textbf{Cílová přesnost:} $5~\%$
    \item \textbf{Amplituda měřicího signálu:} $0{,}1~\mathrm{V}$ -- $2~\mathrm{V}$
    \item \textbf{Stejnosměrná složka (Bias):} $0$ -- $2~\mathrm{V}$
    \item \textbf{Konektivita:} Komunikace s PC a napájení přes USB
\end{itemize}