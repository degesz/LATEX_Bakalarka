\chapter{Implementace analogového obvodu}
...
Specifikace návrhových požadavků definuje, že budicí zdroj musí generovat harmonické 
střídavé napětí s plně nastavitelnými parametry: frekvence, 
amplituda a superponovaná stejnosměrná složka (DC offset).
...
Frekvence výsledného signálu je závislá na prodlevě mezi načítáním jednotlivých hodnot. \cite[str.~451]{ArtOfElectronics}
...
V praxi se používají integrované obvody, které řeší celou funkčnost DDS (např. AD9837 \cite{AD9837}). Vstupní parametr požadované frekvence se do nich zadává 
pomocí sériové sběrnice z mikrokontroléru. 
...
Integrované obvody DDS typicky generují signál s takovou superponovanou stejnosměrnou složkou, že výstupní napětí je vždy kladné. \cite{AD9837}
...
V některých případech je žádoucí k vyfiltrovanému signálu přidat nastavitelnou stejnosměrnou složku. Toho se jednoduše dosáhne \acs{OZ} v zapojení \uv{neinvertující sčítač}, 
...
stejnosměrné složky signálu, tuto úpravu je tedy nutno provést externě. \cite{AD9837}

Digitálního řízení amplitudy lze dosáhnout několika způsoby, například:
\begin{itemize}
    \item Zapojit sinusový signál jako referenci \acs{D/A} převodníku, který pak bude fungovat jako digitálně ovládaný dělič. \cite[str.~413]{ArtOfElectronics}
    \item Použít zesilovač s programovatelným zesílením (\acs{PGA}). Tyto zesilovače většinou mají málo úrovní zesílení. \cite[str.~371]{ArtOfElectronics}
...
\end{equation}
Vstupní amplituda je dána $U_{\text{in}} = 0{,}6\ \unit{V}$, požadovaná maximální amplituda signálu je $U_{\text{out}} = 3\ \unit{V}$.
Dosazením (rovnice \ref{eq:noninv_dosaz}) získáme poměr rezistorů $R_{2}$ a $R_{3}$.
\begin{equation}
    A =\frac{3}{0,6} = 5 = 1 + \frac{4}{1}
    \label{eq:noninv_dosaz}
\end{equation}
Prakticky můžeme použít např. $R_{3} = 10\ \unit{k\Omega}$; $R_{2} = 4\times 10\ \unit{k\Omega}$ v sérii.

Celková amplituda výstupního signálu $U_{\text{out}}$ vychází z rovnice \ref{eq:outputamp}.
Digitální hodnotu potenciometru pro konkrétní amplitudu $U_{\text{out}}$ vypočítáme ze vztahu \ref{eq:digival}.
...
\section{Samovyvažovací můstek}

Ústřední částí měřicího obvodu je samovyvažovací můstek, který se chová jako převodník proudu na napětí. Skládá se z \acs{OZ} a referenčního rezistoru.

\subsection{Výběr operačního zesilovače}
\label{sec:opamp_selection}
Operační zesilovač v měřicí sekci je kritický prvek, odvíjí se od něj přesnost celého měřidla.
...
\begin{enumerate}
    ...
    \acs{GBW} by tedy mělo být alespoň $100\times$ vyšší, než je maximální měřicí frekvence. 
    \item \textbf{Vstupní proud} -- Vstupní proud \acs{OZ} by měl být co nejnižší, aby nedocházelo ke zkreslení zejména při měření vysokých impedancí. \cite{TI_ImpedanceMeasurement} 
    ...
    \end{equation}
    kde $f_{\text{max}}$ je maximální měřená frekvence signálu a $U_p$ je jeho špičková amplituda. \cite[str.~248]{ArtOfElectronics}
...
Pro měření nízkých impedancí se použije menší bočník (např. $1\ \Omega$, $10\ \Omega$), 
pro měření vysokých impedancí se použije větší bočník (např. $1\ \unit{k\Omega}$, $10\ \unit{k\Omega}$).

Pro přepínání bočníků lze použít několik metod:
\begin{itemize}
    \item \textbf{Relé} -- Mechanická relé poskytují nejlepší vlastnosti z hlediska odporu v sepnutém stavu (typicky $< 100\ \unit{m\Omega}$) a izolace v rozepnutém stavu.
    Nevýhodou je jejich pomalé přepínání (typicky 5--10~ms), omezená životnost a vyšší cena. \cite{citrelay_j104d}
...
\begin{tcolorbox}[colback=gray!10, colframe=black!50, boxrule=0.7pt, arc=2pt]
\begin{enumerate}
    \item Začít měření s největším bočníkem (pro nejvyšší impedance).
    \item Změřit napětí na bočníku.
    \item Pokud je napětí pod $10\%$ plného rozsahu \acs{A/D} převodníku, přepnout na menší bočník.
    \item Pokud je napětí nad $90\%$ plného rozsahu, přepnout na větší bočník.
...
%todo  Hodnoty bočníků by měly být voleny tak, aby se jejich rozsahy překrývaly, což umožňuje plynulé přepínání bez mezer v měřitelném rozsahu.

\subsection{Problém se stabilitou obvodu}
\label{sec:stability}

Pokud má měřená impedance kapacitní charakter, může být samovyvažovací můstek náchylný k oscilacím.
Problém nastává ve zpětnovazební smyčce operačního zesilovače, který pracuje jako převodník proudu na napětí. \cite{TI_stability}
%  Jeho přenosová funkce je určena poměrem $\beta = V^-/V_{OUT}$.

Pro zapojení na obr.~\ref{fig:uncompensated} je přenosová funkce zpětné vazby:
\begin{equation}
    \frac{1}{\beta} = \frac{V_{OUT}}{V^-} = 1 + s R_F C_X
    \label{eq:transf}
\end{equation}

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.5\textwidth]{obrazky/uncompensated.pdf}
    \caption{Nekompenzované zapojení pro měření kapacitní impedance.}
...
\begin{equation}
f_Z = \frac{1}{2\pi R_F C_X}
\end{equation}

Tento nulový bod způsobí, že se charakteristiky $|A_{OL}|$ a $|1/\beta|$ protínají s rozdílem strmostí (rate of closure) 40~dB/dekádu (ilustrováno na obr. \ref{fig:uncompensated_graph}), čímž se fázová rezerva snižuje až k~nule a obvod se stává nestabilním.
Frekvence nulového bodu přitom závisí na neznámé kapacitě $C_X$, takže ji nelze kompenzovat pevným kondenzátorem $C_F$ paralelně k~$R_F$ pro celý rozsah měření.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{obrazky/lcr_uncompensated_exact.pdf}
    \caption{Frekvenční závislost $A_{OL}$ a $1/\beta$ pro nekompenzované zapojení.}
    \label{fig:uncompensated_graph}
\end{figure}

\subsubsection{Kompenzace sériovým rezistorem}

Řešení problému je převzato z návodu Texas Instruments \cite{TI_ImpedanceMeasurement}. Kompenzace je provedena přidáním rezistoru $R_G$ do série s měřenou impedancí $Z_X$,
přičemž pro každý měřicí rozsah platí $R_G = R_F$.
Zapojení je znázorněno na obr.~\ref{fig:compensated}.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.5\textwidth]{obrazky/compensated.pdf}
    \caption{Kompenzované zapojení s rezistorem $R_G$ v sérii s měřenou impedancí.}
    \label{fig:compensated}
\end{figure}

V přenosové funkci zpětné vazby se tedy vytvoří pár nula--pól:
\begin{equation}
    \frac{1}{\beta} = \frac{1 + s (R_G + R_F) C_X}{1 + s R_G C_X}.
    \label{eq:beta_comp}
\end{equation}

Kmitočty těchto bodů jsou dány vztahy~\ref{eq:pole_zero}.

\begin{equation}
    f_Z = \frac{1}{2\pi (R_G + R_F) C_X}, \qquad
    f_P = \frac{1}{2\pi R_G C_X}.
    \label{eq:pole_zero}
\end{equation}

Protože $R_G = R_F$, platí, že pól leží vždy na dvojnásobném kmitočtu nuly, a tento poměr je \textbf{nezávislý na $C_X$}.
Pól tak inherentně ruší vliv nuly a rozdíl strmostí charakteristik v~bodě jejich průsečíku se sníží zpět na 20~dB/dekádu (ilustrováno na obr. \ref{fig:compensated_graph}), což zaručuje stabilitu obvodu pro všechny typy a hodnoty měřených impedancí.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{obrazky/lcr_compensated_exact.pdf}
    \caption{Frekvenční závislost $A_{OL}$ a $1/\beta$ pro kompenzované zapojení.}
    \label{fig:compensated_graph}
\end{figure}
