...
\begin{equation}
    \frac{1}{\beta} =\frac{V^-}{V_{OUT}} = 1 + s R_F C_X
    \label{eq:transf}
\end{equation}
...
Tento nulový bod způsobí, že se křivka $A_{ol}\beta$ uzavírá rychlostí 40~dB/dekádu, čímž se fázová rezerva snižuje až k nule a obvod se stává nestabilním.
Frekvence nunulového bodu přitom závisí na neznámé kapacitě $C_X$, takže ji nelze kompenzovat pevným kondenzátorem $C_F$ paralelně k $R_F$ pro celý rozsah měření.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{obrazky/lcr_uncompensated_exact.pdf}
    \caption{Průběh $A_{OL}$ a $1/\beta$ pro nekompenzované zapojení.}
...
Frekvence těchto bodů lze určit z rovnic \ref{eq:pole_zero}.
...
Pól tak inherentně ruší vliv nuly a uzavírací rychlost $A_{ol}\beta$ se sníží zpět na 20~dB/dekádu, což zaručuje stabilitu obvodu pro všechny typy a hodnoty měřených impedancí.

\begin{figure}[h!]
    \centering
    \includegraphics[width=0.8\textwidth]{obrazky/lcr_compensated_exact.pdf}
    \caption{Průběh $A_{OL}$ a $1/\beta$ pro kompenzované zapojení.}
